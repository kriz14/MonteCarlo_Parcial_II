...
public class Punto4_ {

    // Definimos la función que queremos integrar
    public static double f(double x, double y) {
        return Math.sqrt(1- Math.pow(x, 2)); //función
    }
    ...
    public static double monteCarloIntegration(double xMax, double xMin, double yMin, double yMax, int numPoints) {
        ...
        for (int i = 0; i < numPoints; i++) {
            ...
            if (0 <= y && y <= x && Math.pow(y, 2) + Math.pow(x, 2)<= 1){
                series.add(x , y);
                insideRegion += f(x,y);
            }else{
    ...
    public static void main(String[] args) {
        ...
        double xMax = 1;
        double yMin = 0;
        double yMax = Math.PI/4;
        double integralEstimate = monteCarloIntegration(xMin, xMax, yMin, yMax, 100000);
        System.out.println("La aproximacion de la integral es: " + integralEstimate);
    }
}